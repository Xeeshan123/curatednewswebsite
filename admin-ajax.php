...
if(!function_exists('cnews_shared_news1')){
    function cnews_shared_news1(){
        $result = [];

        if(User::is_user_logged_in()){

            if(isset($_POST['user_name']) && isset($_POST['news_id'])){
                $broadcasts=MDB()->query("SELECT * FROM broadcasts WHERE id=".$_POST['user_name']);

                $users=json_decode($broadcasts[0]['users'],true);
